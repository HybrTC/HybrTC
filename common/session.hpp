...
class Session
{
  ...
  public:
    ...
    auto encrypt(const uint8_t* input, size_t input_size) -> v8
    {
        ...
        auto output = crypto.encrypt(input, input_size, *ctr_drbg);

        uint32_t id = *reinterpret_cast<const uint32_t*>(output.data());
        TRACE_ENCLAVE("encrypt %x => len=%lu", id, output.size());
        TRACE_ENCLAVE("encrypt %x => key=%s", id, hexdump(session_key).c_str());
    ...
    {
        uint32_t id = *reinterpret_cast<const uint32_t*>(input);
        TRACE_ENCLAVE("decrypt %x => len=%lu", id, input_size);
        TRACE_ENCLAVE("decrypt %x => key=%s", id, hexdump(session_key).c_str());
